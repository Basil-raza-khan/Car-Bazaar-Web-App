    ...
    return (
        <div>
            ...
            <div className='px-4 md:px-20 my-10'>
                <h2 className='font-bold text-2xl md:text-4xl'>New Listing</h2>
                <form className='p-4 md:p-10 border rounded-xl mt-10 bg-slate-100'>
                    <div>
                        <h2 className='font-bold text-lg md:text-xl mb-6'>Car Detail</h2>
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-5'>
                            {carDetails.carDetails.map((item, index) => (
                                <div key={index}>
                                    <label className='text-sm md:text-lg flex gap-2 items-center mb-2'><IconField icon={item?.icon} /> {item?.label} {item?.required && <span className='text-red-600'>*</span>}</label>
                                    ...
                                    : item.fieldType === 'dropdown' ? <DropDownField
                                        ...
                                        carInfo={carInfo}
                                    />
                                    : item.fieldType === 'textarea' ? <TextAreaField
                                        item={item}
                                        handleInputChange={handleInputChange}
                                        carInfo={carInfo}
                                    />
                    ...
                    <div>
                        <h2 className='font-medium text-lg md:text-xl my-6'>Features</h2>
                        <div className='grid grid-cols-2 md:grid-cols-3 gap-2'>
                            {features.features.map((item, index) => (
                                <div key={index} className='flex gap-2 items-center'>
                                    <Checkbox
                                        onCheckedChange={(value) => handleFeaturesData(item.name, value)}
